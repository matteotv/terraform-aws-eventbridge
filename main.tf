...
resource "aws_cloudwatch_event_rule" "this_lifecycle" {
  for_each = { for k, v in local.eventbridge_rules : v.name => v if var.create && var.create_rules_lifecycle }
  ...
  lifecycle {
    ignore_changes = var.aws_environment == "prod" ? [] : [state]
  }
}
...
resource "aws_scheduler_schedule" "this_lifecyle" {
  for_each = { for k, v in local.eventbridge_schedules : v.name => v if var.create && var.create_schedules_lifecycle }
